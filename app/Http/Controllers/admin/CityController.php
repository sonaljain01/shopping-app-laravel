...
class CityController extends Controller
{
    ...
    {
        $city = City::findOrFail($cityId);
        $city->is_enabled = !$city->is_enabled;  // Toggle delivery status

        $city->save();
        $statusMessage = $city->is_enabled ? 'Delivery Enabled' : 'Delivery Disabled';
        return redirect()->route('admin.city.index')->with('status', 'City delivery status updated to: {$statusMessage}.');
    }
